...
public class StudentEndpointTest {
    ...
    @Before
    public void setup() {
        Client student = new Client(1L, "Legolas", "legolas@lotr.com");
    ...
    public void listStudentsWhenUsernameAndPasswordAreCorrectShouldReturnStatusCode200() {
        List<Client> students = asList(new Client(1L, "Legolas", "legolas@lotr.com"),
                new Client(2L, "Aragorn", "aragorn@lotr.com"));
    ...
    public void getStudentsByIdWhenUsernameAndPasswordAreCorrectShouldReturnStatusCode200() {
        ResponseEntity<Client> response = restTemplate.getForEntity("/v1/protected/students/{id}", Client.class, 1L);
        Assertions.assertThat(response.getStatusCodeValue()).isEqualTo(200);
    }

    @Test
    public void getStudentsByIdWhenUsernameAndPasswordAreCorrectAndStudentDoesNotExistShouldReturnStatusCode404() {
        ResponseEntity<Client> response = restTemplate.getForEntity("/v1/protected/students/{id}", Client.class, -1);
...
//        Assertions.assertThat(exchange.getStatusCodeValue()).isEqualTo(404);
    ...
    public void createWhenNameIsNullShouldReturnStatusCode400BadRequest() throws Exception{
        Client student = new Client(3L, null, "sam@lotr.com");
    ...
    public void createShouldPersistDataAndReturnStatusCode201() throws Exception{
        Client student = new Client(3L, "Sam", "sam@lotr.com");
        BDDMockito.when(studentRepository.save(student)).thenReturn(student);
        ResponseEntity<Client> response = restTemplate.postForEntity("/v1/admin/students/", student, Client.class);
